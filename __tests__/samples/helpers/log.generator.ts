...
export interface LogGeneratorParameters {
  name?: string;
  statusCode?: number;
...
export function generateLog(params: LogGeneratorParameters = {}) {
  params.apiVersion = params.apiVersion === 0 ? 0 : 1;
  const log = {
    [params.apiVersion === 0 ? 'api' : 'service']: {
      name: params.name || availableServices[random(availableServices.length - 1)],
    },
    latencies: {
      proxy: params.proxyLatency || random(2500, 150),
    },
    response: {
      status: params.statusCode || availableStatusCodes[random(availableStatusCodes.length - 1)],
    },
    consumer: {
      username: params.consumerUsername || availableConsumers[random(availableConsumers.length - 1)],
    },
  };
  return merge({ ...templates[params.apiVersion] }, log);
}
